class ChampionshipEngagementsController < ApplicationController
  ...
  def show
    ...
    render json: {
      ...
      published_news: championship.published_news(5).as_json(include: :category),
      featured_partners: championship.featured_partners(6).as_json(include: :category),
      latest_round_selections: championship.latest_round_selections(5).as_json(include: %i[category athletes])
    }
  end
